...
const ModernRegister = () => {
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      ...
      if (!formData.age.trim()) newErrors.age = 'Age is required'; // 🆕 added
            ...
            {[1, 2, 3].map((step) => (
              <React.Fragment key={step}>
                <div className="flex flex-col items-center">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold transition-all duration-300 border-2 ${currentStep >= step
                ...
                {step < 3 && (
                  <div className={`flex-1 h-1 mx-4 rounded-full transition-all duration-300 ${currentStep > step ? 'bg-white shadow-sm' : 'bg-white/20 backdrop-blur-sm'
                    }`} />
                )}
              </React.Fragment>
            ))}
